...
all_data_nans = hea_data.isna().sum(axis=0)

# Ductility Prediction
ductility = hea_data[prop_cols.iloc[0]]
ductility_max = ductility.max()
ductility = ductility / ductility_max
...
regression_models = [linear_reg, ridge_reg, lasso_reg, elastic_net_reg,
                     ...
                     bagging_reg, knn_reg, radius_neighbors_reg, kernel_ridge_reg,
...
model_names = ["Linear Regression", "Ridge Regression", "Lasso Regression",
               ...
               "Radius Neighbors Regression", "Kernel Ridge Regression",
...
X_train, X_test, y_train, y_test = train_test_split(elem_comp_filtered, ductility)
r2_scores = []
r2_scores_train = []

# Iterate through each fold
for train_index, test_index in kf.split(elem_comp_filtered):  # Replace X with your data
    X_train, X_test = elem_comp_filtered.loc[train_index], elem_comp_filtered.loc[test_index]  # Replace X with your data
    y_train, y_test = ductility[train_index], ductility[test_index]  # Replace y with your target variable
...
r2_scores_max = r2_scores.max(axis=1)